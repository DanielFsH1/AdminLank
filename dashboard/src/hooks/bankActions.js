...
export async function createCreditAccount(bankId, creditData) {
  ...
  const credit = {
    ...
    daysAfterCutoff: Math.min(30, Math.max(10, parseInt(creditData.daysAfterCutoff, 10) || 20)),
...
export async function updateCreditAccount(bankId, creditData) {
  ...
  const updated = {
    ...
    daysAfterCutoff: Math.min(30, Math.max(10, parseOptionalInt(creditData.daysAfterCutoff, current.daysAfterCutoff) ?? 20)),
...
export async function migrateExistingBanks() {
  ...
  creditAccounts.forEach(ca => {
    ...
    bankMap[id].creditAccount = {
      ...
      daysAfterCutoff: ca.daysAfterCutoff || (ca.paymentDueDay && ca.cutoffDay ? Math.min(30, Math.max(10, ca.paymentDueDay - ca.cutoffDay + (ca.paymentDueDay <= ca.cutoffDay ? 30 : 0))) : 20),
