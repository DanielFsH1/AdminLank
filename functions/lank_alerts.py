...
def generate_renewal_alerts(db, services_config=None):
    """
    Revisa todos los grupos activos de servicios con isRenewalBased: true.
    Para cada usuario que tenga renewDay == día actual, genera una alerta
    de renovación si no existe una pendiente para ese usuario.
    Retorna el número de alertas generadas.
    """
    from datetime import date, timedelta, timezone as tz
    # Usar zona horaria de México (UTC-6) para el día actual
    # Cloud Functions corre en UTC; el admin opera en México
    mexico_tz = tz(timedelta(hours=-6))
    today_day = datetime.now(mexico_tz).day

    # Determinar qué servicios son renewal-based
    renewal_services = {}  # key → service_name
    if services_config:
        for key, svc in services_config.items():
            if svc.get('active') is False:
                continue
            if svc.get('isRenewalBased'):
                renewal_services[key] = svc.get('name', key)
    else:
        # Fallback: solo microsoft365
        renewal_services = {'microsoft365': 'Microsoft 365'}

    if not renewal_services:
        return 0
    ...
    for service_key, service_name in renewal_services.items():
        ...
        for group_doc in groups:
            ...
            for user in users:
                ...
                if renew_day != today_day:
                    continue

                # Verificar que no exista una alerta pendiente duplicada
                ...
                email_info = f' ({invitation_email})' if invitation_email else ''
                ...
                alert = {
                    ...
                    'currentRenewDay': renew_day,
                    'title': f'Renovar {service_name} — {user_alias}',
                    'description': (
                        f'Hoy es dia {renew_day}: renovar {service_name} para {user_alias}{email_info} '
                        f'del grupo #{account_id} {account_alias}.'
                    ),
                    'createdAt': now,
                    'completedAt': None,
...
def generate_missing_renewal_alerts(db, services_config=None):
    """
    Revisa todos los grupos activos de servicios con isRenewalBased: true.
    Para cada usuario que NO tenga renewDay, genera una alerta 'medium'
    indicando que falta el día de renovación.
    Retorna el número de alertas generadas.
    """
    # Determinar qué servicios son renewal-based
    renewal_services = {}  # key → service_name
    ...
    for service_key, service_name in renewal_services.items():
        ...
        for group_doc in groups:
            ...
            for user in users:
                ...
                renew_day = user.get('renewDay')
                if renew_day:
                    continue  # Ya tiene día de renovación

                alert_type = f'{service_key}_missing_renewal'
                if find_duplicate(existing_alerts, alert_type, service_name,
                                  str(account_id), user_alias):
                    continue
                ...
                alert = {
                    ...
                    'userAlias': user_alias,
                    'invitationEmail': invitation_email,
                    'title': f'Falta fecha de renovacion — {user_alias}',
                    'description': (
                        f'{user_alias}{email_info} del grupo #{account_id} {account_alias} '
                        f'no tiene dia de renovacion configurado para {service_name}. '
                        f'Se necesita este dato para el funcionamiento completo del servicio.'
                    ),
                    'createdAt': now,
                    'completedAt': None,
                    'source': 'cloud_scheduled',
...
def generate_m365_renewal_alerts(db):
    return generate_renewal_alerts(db)
